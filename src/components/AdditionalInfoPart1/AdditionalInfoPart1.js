...
const AdditionalInfoPart1 = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('data', new Blob([JSON.stringify(nicknameData)], { type: 'application/json' }));
        if (profileImg) {
            formData.append('profileImg', profileImg);
        }
        try {
            const accessToken = localStorage.getItem('accessToken');
            const response = await axios.post('http://localhost:8080/auth/additional-info-part1', formData, {
                headers: {
    ...
    return (
        <div className={styles.container}>
            ...
            <main className={styles.main}>
                ...
                <p className={styles.subtitle}>반가워요! 사용할 닉네임과 프로필 사진을 설정해주세요.</p>
                <form onSubmit={handleSubmit} className={styles.form}>
                    <div className={styles.profilePicContainer}>
                        <div className={styles.avatar}>
                            {previewImg ? <img src={previewImg} alt="Profile preview" className={styles.profileImage} /> : <DefaultAvatar width={120} height={120} />}
                        </div>
                        <input
                            type="file"
                            accept="image/*"
                            ...
                            style={{ display: 'none' }}
                        />
                        <button type="button" className={styles.editButton} onClick={handleImageUploadClick}>
                            <PencilIcon />
                        </button>
                    </div>
                    ...
                    <button type="submit" className={styles.submitButton} disabled={!isNicknameValid()}>
                        다음
                    </button>
                </form>
            </main>
